...
package fr.steph.kanji.core.ui.util.furiganaview
...
class FuriganaTextView : AppCompatTextView {
    ...
    fun setFuriganaText(text: String, hasRuby: Boolean = false) {
        super.setText(text)
    ...
    private fun setText(tp: TextPaint, text: String, markS: Int, markE: Int) {
        ...
        // Sizes
        lineSize = textPaintFurigana.fontSpacing + textPaintNormal.fontSpacing.coerceAtLeast(0f)
    ...
    override fun onMeasure(widthMs: Int, heightMs: Int) {
        // Modes
        val wmode = MeasureSpec.getMode(widthMs)
        val hmode = MeasureSpec.getMode(heightMs)

        // Dimensions
        val wold = MeasureSpec.getSize(widthMs)
        val hold = MeasureSpec.getSize(heightMs)
        ...
        // New height
        var hnew = Math.round(ceil((lineSize * normalLines.size.toFloat()).toDouble())).toInt()
        var wnew = wold
        if (wmode != MeasureSpec.EXACTLY && normalLines.size <= 1)
            wnew = Math.round(ceil(lineMax.toDouble())).toInt()
